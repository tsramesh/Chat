...
void extract_command(const unsigned char *buffer, char *command, unsigned char **data, size_t *data_len) {
    // Copy the first 4 bytes as the command
    memcpy(command, buffer, 4);
    command[4] = '\0';

    // Remaining data is the payload
    *data_len = strlen((const char *)(buffer + 4));
    *data = (unsigned char *)(buffer + 4);
}

void handle_chat(int client_fd, unsigned char *data, size_t data_len) {
    int target_fd = atoi((const char *)data); // Assuming the target client FD is sent
...
void handle_command(int client_fd, const char *command, const unsigned char *data, size_t data_len) {
...
void handle_file(int client_fd, const unsigned char *data, size_t data_len) {
    return;
}

void handle_conf(int client_fd, const unsigned char *data, size_t data_len) {
    return;
}

void handle_auth(int client_fd, const unsigned char *data, size_t data_len) {
    return;
}

void broadcast_message(const unsigned char *message, size_t message_len, int sender_fd, const int *client_fds, size_t num_clients) {
    return;
}

FileContent *readFile(const char *path) {
